...
export function useWhiteCrossLessonStep(studentFrame: CubeState | null) {
  ...
  const isComplete = useCallback((step: WhiteCrossLessonStep | null) => step?.kind === 'complete', [])
  const countProgress = useCallback((frame: CubeState) => countSolvedCrossSlots(frame), [])
