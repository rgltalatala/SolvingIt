...
function slotsGainedForCandidate(
  ...
  if (!('demoMoves' in candidate.step) || !candidate.step.demoMoves?.length)
    return 0;
  return slotsGainedAfterDemo(
    studentState,
 ...
 */
export function tryPermuteReadyPass(
  studentState: CubeState,
  ...
  for (const candidates of tiers) {
    ...
    if (
      best.step.kind === 'side-connect' &&
      slotsGainedForCandidate(studentState, best) === 0
    ) {
      continue;
    }
    return best.step;
