...
export function partnerColorForSlot(
...
export function slotSolved(state: CubeState, id: CrossEdgeId): boolean {
  ...
  return (
    ...
    state[slot.sideFace][slot.sideIndex] === center
  );
}
...
export function crossSlotIdForPartner(
  studentState: CubeState,
  partner: Color,
): CrossEdgeId | null {
...
export function crossSlotsSolvedInState(
  studentState: CubeState,
): CrossEdgeId[] {
  return CROSS_ORDER.filter((id) => slotSolved(studentState, id));
}

export function crossSlotsToPreserve(
  studentState: CubeState,
  exceptId?: CrossEdgeId,
): CrossEdgeId[] {
  return crossSlotsSolvedInState(studentState).filter((id) => id !== exceptId);
}

export function unsolvedWhiteOnDSlotIds(
  studentState: CubeState,
): CrossEdgeId[] {
...
export function slotShowsRotateBottomPattern(
  studentState: CubeState,
  id: CrossEdgeId,
): boolean {
  if (slotSolved(studentState, id)) return false;
  ...
  return (
    ...
    studentState[slot.sideFace][slot.sideIndex] !==
