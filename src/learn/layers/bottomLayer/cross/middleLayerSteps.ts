...
export function middleLayerConnectDemoMove(
  ...
  for (const move of [clockwiseMove, counterClockwiseMove]) {
    ...
    const preservesSolvedSlots = solvedSlotsToPreserve.every((id) => slotSolved(nextState, id))
    if (!preservesSolvedSlots) continue
    const rank = rankEdgeAfterMove(nextState)
    if (rank < 0) continue
    if (!best || rank > best.rank || (rank === best.rank && move === clockwiseMove)) {
      best = { move, rank }
    }
  }
...
export function collectMiddleLayerPermuteCandidates(studentState: CubeState): PermuteReadyCandidate[] {
  const crossPartners = new Set(CROSS_ORDER.map((id) => partnerColorForSlot(studentState, id)))
